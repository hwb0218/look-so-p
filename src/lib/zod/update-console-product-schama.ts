...
export const updateProductFormSchema = z.object({
  thumbnail: z
    ...
    .refine((files) => Array.from(files as FileList).every((file) => file.size <= MAX_IMAGE_SIZE))
    .refine((files) => Array.from(files as FileList).every((file) => ALLOWED_IMAGE_TYPES.includes(file.type)))
    .optional(),
  images: z
    .custom<FileList>((val) => val instanceof FileList, '썸네일 또는 보조 이미지를 추가하세요')
    .refine((files) => files.length <= 4, '촤대 3장의 보조 이미지만 가능합니다')
    .refine(
      (files) => Array.from(files as FileList).every((file) => file.size <= MAX_IMAGE_SIZE),
      '최대 이미지 크기는 5MB입니다',
    )
    .refine((files) =>
